...
export const handler = async (client: NextbillionSDK, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.mdm.getDistanceMatrixStatus(body)));
  } catch (error) {
    if (isJqError(error)) {
