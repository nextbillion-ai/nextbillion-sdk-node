...
export interface MdmCreateDistanceMatrixParams {
   ...
   * country will be preferred if they are feasible for the given set of
   * `destination` or `waypoints` . When set to true, the routes will be allowed to
   ...
   * be used to provide the departure time. The response will return the `distance`
   * and `duration` of the route based on typical traffic for at the given start
   * time.If no input is provided for this parameter then the traffic conditions at
  ...
  /**
   * Body param: Specify the side of the road from which to approach `origins`
   * points. Please note that the given approach will be applied to all the points
   * provided as `origins`.
   */
